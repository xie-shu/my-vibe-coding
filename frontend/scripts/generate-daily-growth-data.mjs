...
const FRONTEND_ROOT = existsSync('frontend/public') ? 'frontend' : '.'
const OUT_FILE = path.resolve(FRONTEND_ROOT, 'public/daily-growth.json')
...
async function logAvailableModels() {
  ...
  try {
    ...
    const modelIds = Array.isArray(payload.data)
      ? payload.data.map((item) => item?.id).filter(Boolean).slice(0, 30)
      : []
    console.warn('Models available to this API key:', modelIds.join(', ') || '(none returned)')
  } catch {
    // Model discovery is diagnostic only and must not fail the build.
  }
}
  ...
  const repos = await Promise.all(
    REPOS.map(async (fullName) => {
      try {
        ...
        console.warn(`GitHub fetch failed for ${fullName}, using fallback metadata: ${error.message}`)
        ...
        return {
          repo: {
            ...
            description: 'GitHub AI 项目资料源。构建时 GitHub API 暂时不可用，页面使用项目白名单生成兜底热点内容。',
            stargazers_count: 0,
