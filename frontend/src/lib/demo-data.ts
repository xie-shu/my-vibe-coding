...
const STORAGE_KEY = 'growth-workbench-core-state-v1'
const GROWTH_STORAGE_KEY = 'growth-workbench-demo-state-v8-github'
...
export async function demoGetGrowthToday(): Promise<GrowthToday> {
  ...
  const question = growth.questions.find((item) => item.status === 'new') || growth.questions[0]
...
export async function demoListDailyQuestions() {
  await pause()
  return clone(readGrowthState().questions)
}
...
export async function demoListRadarItems(tag?: string) {
  await pause()
  const items = readGrowthState().radarItems
...
function buildDemoAnswer(query: string) {
  const growth = readGrowthState()
  const todayQuestion = growth.questions.find((question) => question.status === 'new') || growth.questions[0]
