...
function buildDemoAnswer(query: string) {
  ...
  if (/AI产品经理|AI 产品经理|需要懂什么|能力模型|核心能力/.test(query)) {
    return `AI 产品经理需要同时理解“用户问题”和“AI 能力边界”。可以按四层能力来讲：\n\n1. 产品基本功：用户、场景、需求、MVP、指标和迭代。\n2. AI 理解力：知道大模型、RAG、Agent、多模态、Prompt、工具调用分别适合解决什么问题。\n3. 体验与风险判断：能设计兜底、人工确认、可解释、可编辑和失败处理。\n4. 数据与评估：能用命中率、采纳率、完成率、满意度、Badcase 等指标判断 AI 功能是否真的有效。\n\n面试表达可以说：AI PM 不是把模型接进产品，而是把不稳定的模型能力包装成可控、可验证、能持续优化的用户任务闭环。`
  }
  if (/什么是AI产品|什么是 AI 产品|AI产品是什么|AI 产品是什么|大模型产品|LLM产品/.test(query)) {
    return `AI 产品不是“接了大模型”的普通功能，而是把模型能力嵌入到用户任务里，让用户更快完成某件事。\n\n判断一个 AI 产品是否成立，可以看三点：\n\n1. 用户任务是否真实：它解决的是高频、复杂或高成本任务吗？\n2. AI 是否带来明显增益：相比传统流程，是更快、更准、更省力，还是更个性化？\n3. 产品闭环是否完整：有没有输入、生成、编辑、确认、反馈、评估和兜底？\n\n比如这个成长工作台里，AI 的价值不是“聊天”，而是把热点资料转成练习题，再对回答做结构化反馈。`
  }
  if (/用户画像/.test(query)) {
  ...
  if (/Prompt|提示词|提示工程|system prompt|系统提示/.test(query)) {
    return `Prompt 不是简单“写一句指令”，而是把任务目标、输入信息、输出格式、约束边界和失败处理讲清楚。\n\n做 AI 产品时，Prompt 设计通常要关注：\n\n1. 角色：模型以什么身份回答。\n2. 任务：要完成什么具体动作。\n3. 上下文：可以参考哪些资料，哪些不能编造。\n4. 输出格式：是否需要结构化字段、表格或 JSON。\n5. 边界：资料不足、工具不可用、风险场景时怎么兜底。\n\n面试里可以强调：Prompt 是产品规则的一部分，不只是技术调参。`
  }
  if (/幻觉|胡说|乱编|不准确|可信|可靠/.test(query)) {
    return `AI 幻觉是指模型生成了看似合理但没有事实依据的内容。产品上不能只靠“换更强模型”解决，而要做系统性设计。\n\n常见方案包括：\n\n1. 接入知识库或工具，让回答有依据。\n2. 展示引用来源，让用户可追溯。\n3. 对低置信度问题触发澄清或拒答。\n4. 关键内容支持人工确认和编辑。\n5. 建立 Badcase 反馈，用错误样本持续优化。\n\n这个工作台里，天气这类实时问题如果没有工具，就应该明确说不能查，而不是编一个天气。`
  }
  if (/指标|如何评估|评估.*AI|效果.*衡量|成功指标|北极星/.test(query)) {
    return `评估 AI 产品不能只看“用户觉得好不好”，要拆成任务效果、AI 质量和业务结果三层。\n\n可以这样设计指标：\n\n1. 任务完成：完成率、平均耗时、一次成功率。\n2. AI 质量：答案采纳率、引用准确率、幻觉率、Badcase 率。\n3. 用户体验：修改次数、追问次数、满意度、复用率。\n4. 业务结果：留存、转化、付费、效率提升。\n\n比如 AI 问答助手可以看：基础问题命中率、知识库引用准确率、用户是否采纳回答、错误反馈是否下降。`
  }
  if (/多模态|图片|语音|视频|视觉/.test(query)) {
    return `多模态是指 AI 能处理文本以外的信息，比如图片、语音、视频或文档版式。产品价值不在于“支持更多输入”，而在于降低用户表达成本。\n\n产品设计时可以问：\n\n1. 用户为什么不直接打字？\n2. 图片/语音里有哪些关键信息需要被理解？\n3. 输出结果是否需要用户确认和编辑？\n4. 识别失败时是否有补救路径？\n\n例如面试训练里，语音回答能还原真实表达，但如果 ASR 不稳定，就应该允许用户编辑转写文本。`
  }
  if (/Function calling|函数调用|工具调用|调用工具|tool/.test(query)) {
    return `工具调用是让模型在需要外部能力时调用确定性的工具，例如查天气、查数据库、发邮件、检索文档。它的产品价值是把“会说”变成“能做”。\n\n设计工具调用要注意：\n\n1. 哪些问题必须调用工具，哪些可以直接回答。\n2. 工具参数如何校验，避免误操作。\n3. 工具失败时如何兜底。\n4. 高风险操作是否需要用户确认。\n5. 调用结果是否要展示来源和时间。\n\n所以用户问天气时，如果没有天气工具，助手就应该明确说无法查询。`
  }
  if (/PRD|需求文档|需求分析|功能需求/.test(query)) {
    return `写 AI 产品 PRD 时，除了普通产品的背景、目标、用户、流程和功能，还要额外写清 AI 能力边界。\n\n建议结构是：\n\n1. 产品背景和目标。\n2. 用户场景和核心痛点。\n3. 功能流程和页面交互。\n4. AI 输入、输出、上下文来源。\n5. 异常兜底和人工确认。\n6. 效果指标和评估方法。\n7. 版本边界：当前做什么，后续优化什么。\n\nAI 产品 PRD 最怕只写“调用大模型生成结果”，但没有说明结果错了怎么办。`
  }
  if (/竞品|竞品分析|对比/.test(query)) {
    return `竞品分析不要只列功能表，更重要的是回答“为什么它这样做”。\n\n可以按这个框架：\n\n1. 目标用户：服务谁？\n2. 核心场景：用户什么时候用？\n3. 关键功能：解决了什么任务？\n4. AI 能力：用了生成、检索、Agent、多模态还是工具调用？\n5. 体验闭环：是否可编辑、可追溯、可反馈？\n6. 商业化和壁垒：数据、工作流、生态还是品牌？\n\n面试里要把竞品分析落到你自己的产品取舍，而不是停在截图罗列。`
  }
  if (/产品思维|怎么提升|练习产品|产品判断/.test(query)) {
    return `产品思维可以理解为：面对一个模糊问题，能判断用户是谁、问题是否真实、方案是否值得做、效果如何验证。\n\n练习时可以固定用这个结构：\n\n1. 用户：谁遇到这个问题？\n2. 场景：什么时候、为什么发生？\n3. 痛点：现在的成本或阻碍是什么？\n4. 方案：MVP 先做哪条闭环？\n5. 指标：如何证明它有效？\n6. 边界：哪些不做，失败怎么兜底？\n\n这个平台每天生成练习题，本质上就是帮你反复训练这套判断链路。`
  }
  if (/AI|产品|大模型|模型|智能助手|问答助手|知识库|Agent|RAG|MVP|Prompt|指标|需求|用户/.test(query)) {
    return `这是一个 AI 产品相关问题，我建议按“场景—问题—方案—指标—边界”来分析。\n\n你可以这样回答：\n\n1. 先明确用户和使用场景，避免一上来讲技术。\n2. 再说明当前痛点，比如信息过载、决策成本高、表达训练缺反馈。\n3. 然后讲 AI 能力如何进入流程，例如生成、检索、总结、推荐、评估或工具调用。\n4. 接着给出 MVP 闭环，说明第一版先验证什么。\n5. 最后补充指标和风险兜底，比如采纳率、准确率、人工确认、引用来源和失败提示。\n\n如果你把问题再说具体一点，比如“如何设计 RAG 问答助手”或“怎么评估 AI 产品效果”，我可以继续展开成面试话术。`
  }
  return `这个问题如果是普通常识，我可以直接回答；如果是 AI 产品、产品经理、RAG、Agent、MVP、Prompt、指标、需求分析等方向，你可以直接问，我会按产品经理面试表达来拆解。\n\n如果问题涉及天气、股价、航班、实时新闻等外部实时信息，就需要对应工具；当前没有工具时我会明确兜底，不会编造。`
}
